<template>
  ...
  >
    <div class="absolute inset-0 z-0">
      <video
        v-if="heroBanner?.type === 'video' && mediaUrl"
        autoplay
        muted
        loop
        playsinline
    ...
    <div class="relative z-10 container mx-auto px-4 text-center text-white">
      ...
      <router-link
        to="/donate"
        class="inline-flex items-center justify-center px-8 py-6 text-lg rounded-lg bg-primary text-primary-foreground hover:opacity-90"
        >Donasi Sekarang</router-link
      >
    </div>
    <div class="absolute bottom-8 left-1/2 -translate-x-1/2 z-10">
      <div
        class="w-6 h-10 border-2 border-white rounded-full flex items-start justify-center p-2"
      >
...
const mediaUrl = computed(() =>
  heroBanner.value?.media_url
    ? resolveMediaUrl(heroBanner.value.media_url)
    : undefined,
);
const bgStyle = computed(() => ({
  backgroundImage:
    heroBanner.value && mediaUrl.value
      ? "url(" + mediaUrl.value + ")"
      : "linear-gradient(to bottom, #1e3a8a, #3b82f6)",
}));
onMounted(async () => {
  banners.value = await bannerApi.getAll(1, 0);
});
</script>
