    ...
    <div class="container mx-auto px-4">
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        <div>
          ...
          <p v-if="subtitle" class="text-lg text-blue-700 mb-4">{{ subtitle }}</p>
          <p class="text-lg text-gray-600 mb-4">{{ description }}</p>
          <p v-if="additionalDescription" class="text-lg text-gray-600 mb-4">{{ additionalDescription }}</p>
          <p v-if="error" class="text-sm text-red-600 mt-2">{{ error }}</p>
          <div class="grid grid-cols-2 gap-4 mt-8">
            <div class="bg-blue-50 rounded-lg p-4">
              <h3 class="text-2xl font-bold text-blue-600">{{ yearsActive }}+</h3>
              <p class="text-gray-600">Tahun Berdiri</p>
            </div>
            <div class="bg-green-50 rounded-lg p-4">
              <h3 class="text-2xl font-bold text-green-600">{{ activeMembers }}+</h3>
        ...
        <div class="relative h-96 rounded-lg overflow-hidden shadow-xl">
          <Transition name="slide-fade" mode="out-in">
            <img :key="currentImage" :src="currentImage" :alt="title" class="absolute inset-0 w-full h-full object-cover" />
          </Transition>
        </div>
      </div>
    </div>
...
const currentImage = computed(() => images.value[currentIndex.value] || FALLBACK)
const title = computed(() => about.value?.title || 'Tentang Masjid Al-Madr')
const subtitle = computed(() => about.value?.subtitle || '')
const description = computed(() => about.value?.description || '')
const additionalDescription = computed(() => about.value?.additional_description || '')
...
onMounted(async () => {
  ...
  if (images.value.length > 1) t = setInterval(() => { currentIndex.value = (currentIndex.value + 1) % images.value.length }, 4000)
})
onUnmounted(() => { if (t) clearInterval(t) })
</script>

<style scoped>
.slide-fade-enter-active, .slide-fade-leave-active { transition: opacity 1.2s ease, transform 1.2s ease; }
